...
def dijkstra(graph_1d,n,start):
    ...
    for i in range(0,n):
        a.append(start)
        index_start = int(get_index(start,i,n))
        length.append(graph_1d[index_start])
        distance.append(graph_1d[index_start])
        pass
    length[start] = -1    
    for steps in range(0,n-1):
        ...
        F.append(edge)        
        for i in range(0,n):
            index_vclose = int(get_index(vclose,i,n))
            if length[vclose] + graph_1d[vclose][i] < length[i]:
                length[i] = length[vclose] + graph_1d[index_vclose]
                distance[i] = length[vclose] + graph_1d[index_vclose]
                a[i] = vclose
        length[vclose] = -1
    for item in F:
        item[0] += 1
